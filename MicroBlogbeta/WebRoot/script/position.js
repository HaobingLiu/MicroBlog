...
function select() {
	with(document.regform.province) { var loca2 = options[selectedIndex].value; }
	for(i = 0;i < where.length;i ++) {
		if (where[i].loca == loca2) {
			loca3 = (where[i].locacity).split("|");
			for(j = 0;j < loca3.length;j++) { with(document.regform.city) { 
				length = loca3.length; options[j].text = loca3[j]; options[j].value = loca3[j]; var loca4=options[selectedIndex].value;}}
				break;
		}
	}
	document.regform.u_position.value=loca2+loca4;
} 

function init() {
	with(document.regform.province) {
	length = where.length;
	for(k=0;k<where.length;k++) { options[k].text = where[k].loca; options[k].value = where[k].loca; }
	options[selectedIndex].text = where[0].loca; options[selectedIndex].value = where[0].loca;
	}
	with(document.regform.city) {
...
function init1() {
	with(document.updform.province) {
		length = where.length;
...
function select1() {
	...
	document.updform.u_position.value=loca2+loca4;
} 
