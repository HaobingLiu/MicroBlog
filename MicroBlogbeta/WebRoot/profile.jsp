...
<title>微博 - ${u_nickname}的微博</title>
 <%
 	User user=(User)session.getAttribute("user");
  	...
  	List<Mood> lstMMod=showmooddao.showMyMood(user.getU_id(),pageNo,8);//showMyMood
  	pageContext.setAttribute("lstMMod",lstMMod);
  	//要推荐的好友，为好友的好友
...
<%
  if(request.getParameter("msg")!=null)
  {
   ...
   {
     case 3:
     out.print("<script>alert('广播成功！'); </script>");
     break;
     case 4:
     out.print("<script>alert('广播失败，请检查是否上传了配图！'); </script>");
     break;
...
<table border="0" align="center" cellpadding="0" cellspacing="0" id="container">
	<tr>
		<td width="670" height="600" valign="top">
			...
			<!-- weibo 开始-->
			<c:forEach items="${lstMMod}" var="lstMMod">
			<table id="weibo" width="90%" border="0" align="center" cellpadding="3" cellspacing="0" height="120">
				<tr>
					<td width="88%" class="content">${lstMMod.m_content }</td>
				</tr>
				<tr>
					<td>
					  <c:if test="${lstMMod.m_images!=null}">
					  <img src="${lstMMod.m_images}"  width="120" height="120"/>
					  </c:if>
					</td>
				</tr>
				<tr>
					<td height="25">
						<table width="100%" border="0" cellpadding="0" cellspacing="0" id="weibo_status">
							<tr>
								<td>${lstMMod.m_date} </td>
								<td align="right"><a href = "servlet/RemoveMoodServlet?m_id=${lstMMod.m_id}">删除</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a href="servlet/DealTransmit?u_id=${u_id}&m_id=${lstMMod.m_id}&type=2">赞</a>(${lstMMod.m_transmit_num})</td>
							</tr>
						</table>
					</td>
				</tr>
				<tr>
					...
					<td>
						<form id="formcomment" name="formcomment" method="post" action="servlet/DealCommentServlet">
						  ...
						  <input name="u_id" value="${u_id }" type="hidden"/>
						  <input name="m_id" value="${lstMMod.m_id }" type="hidden"/>
						  <input name="button6" type="submit" id="button6" value="评论" />
						</form>
						<form id="formshowcomment" name="formshowcomment" method="post" action="servlet/getCommentServlet?m_id=${lstMMod.m_id }">
			...
			<% int all=(findOrder.getMyNum(user.getU_id())-1)/8+1; %>
			<table align="right"> 
				<tr>
		...
		<td width="280" align="center" valign="top" class="pageright">
			...
			<table border="0" align="center" cellpadding="0" cellspacing="0" id="userlist">
				<tr>
					...
					<td align="right" class="title"><a href="profile.jsp?No=<%=pageNo %>">[换一换]</a></td>
				</tr>
